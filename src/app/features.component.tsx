...
export default function HeaderMenu() {
  ...
  return <div className={ `flex flex-col gap-3 relative ${ active ? 'bg-primary' : '' }` }>
    <div className="flex flex-col gap-3 pt-10 px-10">
      <aside className="flex flex-row gap-3 items-center px-[20px]">
        <h1>
        ...
        <div className="flex flex-row gap-3">
          ...
          <button onClick={ () => setStatus( !active ) } className={ `py-3 px-5 flex text-left rounded-full transition-background text-xs text-primary-foreground bg-primary hover:bg-primary-500` }>
            <span className="material-icon">{ active ? 'close' : 'menu' }</span>
          </button>
        </div>
      </aside>
    </div>
    { active && <div className="absolute top-[100%] w-full flex flex-row bg-primary p-10 z-50 gap-6 justify-center">
...
function Feat({ title, details, link }: { title: string, details: string, link?: string }){
  return <div className="flex flex-col p-6 bg-foreground-900 aspect-[192/270] w-48 rounded-[2.5rem] transition-background hover:bg-primary-500 gap-3">
